// Adapted from https://github.com/michael/github/blob/master/github.js for Google Apps Scripts
...
var _ = Underscore.load();
  ...
  var Github = function(options) {
    ...
    function _request(method, path, data, cb, raw, sync) {
      ...
      var response = UrlFetchApp.fetch(getURL(), params);
      
      var status = response.getResponseCode();
      var responseText = response.getContentText();
      
      if (status >= 200 && status < 300 || status === 304) {
        cb(null, raw ? responseText : responseText ? JSON.parse(responseText) : true, this);
      } else {
